...
function commit_sauvegarde($type_sauvegarde)
{
    ...
    echo getDataFile($type_sauvegarde);
}
 ...
 */
function sauvegardeAsFile($previousVersion, $newVersion) : void
{
    $typeSauvegarde = 'all';
    $contentFile = getDataFile($typeSauvegarde);
 ...
 * @return string
 */
function getDataFile($typeSauvegarde) : string
{
    $content = "#\n";
    $content .= "# Libertempo\n";
    ...
    while ($resultat = $ReqLog->fetch_array()) {
        ...
        if(($typeSauvegarde=="all") || ($typeSauvegarde=="structure") ) {
            $content .= "# Struture : \n#\n";
            $content .= get_table_structure($table);
        }
        if(($typeSauvegarde=="all") || ($typeSauvegarde=="data") ) {
            $content .= "# Data : \n#\n";
            $content .= get_table_data($table);
        }
    }
...
// CHOIX
function choix_save_restore() : void
{
    $PHP_SELF = filter_input(INPUT_SERVER, 'REQUEST_URI', FILTER_SANITIZE_URL);
    $return   = '';
